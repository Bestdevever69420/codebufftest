...
public class WorldGenPyroxene2Tree extends WorldGenAbstractTree {
    private static final IBlockState LOG = ModBlocks.pyroxene2_wood.getDefaultState().withProperty(BlockLog.LOG_AXIS, BlockLog.EnumAxis.Y);
    private static final IBlockState LEAVES = ModBlocks.pyroxene2_leaves.getDefaultState();

    public WorldGenPyroxene2Tree(boolean notify) {
        super(notify);
    }

    public boolean generate(World worldIn, Random rand, BlockPos position) {
        int height = rand.nextInt(4) + 6; // Slightly taller than Pyroxene1
        boolean flag = true;
        ...
        if (!flag) {
            ...
            if (isSoil && position.getY() < 256 - height - 1) {
                ...
                // Larger canopy for Pyroxene2 trees
                for (int i3 = -3; i3 <= 3; ++i3) {
                    for (int l3 = -3; l3 <= 3; ++l3) {
                        if ((i3 > -3 && i3 < 3 || l3 > -3 && l3 < 3) && (i3 == 0 && l3 == 0 ? false : rand.nextInt(3) != 0)) {
                            this.setBlockAndNotifyAdequately(worldIn, position.add(i3, height - 1, l3), LEAVES);
                            this.setBlockAndNotifyAdequately(worldIn, position.add(i3, height - 2, l3), LEAVES);
                        }
                    }
                }
                ...
                for (int k3 = -1; k3 <= 1; ++k3) {
                    for (int l3 = -1; l3 <= 1; ++l3) {
                        if (rand.nextInt(2) == 0) {
                            this.setBlockAndNotifyAdequately(worldIn, position.add(k3, height - 3, l3), LEAVES);
                        }
                    }
                }
                return true;
